...
class Command(object):
    ...
    def get_version(self):
        return "servos version is %s" % servos.version
    ...
    def get_services(self, global_options):
        from servos.core.simpleframe import get_services
        return get_services(global_options.services_dir,
    ...
    def get_application(self, global_options):
        from servos.manage import make_simple_application

        return make_simple_application(services_dir=global_options.services_dir,
    ...
    def execute(self, args, options, global_options):
        from servos.utils.common import check_services_dir

        # add services_dir to global_options and insert it to sys.path
        if global_options.services_dir not in sys.path:
            sys.path.insert(0, global_options.services_dir)

        if self.check_services_dirs:
            check_services_dir(global_options.services_dir)
        if self.check_services and args:  # then args should be services
            all_services = self.get_services(global_options)
            services = args
            args = []
            for p in services:
                if p not in all_services:
                    print 'Error: ServiceName %s is not a valid service' % p
                    sys.exit(1)
                else:
                    args.append(p)
        try:
...
class CommandManager(Command):
    ...
    def execute(self, callback=None):
        ...
        if not self.global_options:
            ...
            global_options.services_dir = os.path.normpath(
        ...
        if subcommand == 'help':
            if len(args) > 1:
                ...
                if issubclass(command, CommandManager):
                    cmd = command(['help'], None, '%s %s' % (
                        self.prog_name, args[1]), global_options=global_options)
                    cmd.execute()
                else:
        ...
        if global_options.help:
            ...
            if issubclass(command, CommandManager):
                cmd = command(args[1:], None, '%s %s' % (
                    self.prog_name, subcommand), global_options=global_options)
                cmd.execute()
            else:
                cmd = command()
                cmd.run_from_argv(
                    self.prog_name, subcommand, global_options, args[1:])
