...
r_var_env = re.compile(
    ur'(?<!\{)\{\{([^\{].*?)(?<!\})\}\}(?!\})|(?:\$(\w[\d\w_]*)|\$\{(\w[\d\w_]*)\})', re.U)
__default_env__ = {}
...
def merge_data(values, prev=None):
    ...
    for value in values:
        ...
        if prev and _type != type(value):
            raise ValueError(
                "Value %r should be the same type as previous type %r" % (value, _type))
    ...
    if isinstance(a, (list, tuple)):
        ...
        for i, k in enumerate(a):
            if beautiful:
                ...
                ind = indent
            s.append(indent_char * ind + uni_prt(k, encoding,
                                                 beautiful, indent + 1,
        ...
        for i, k in enumerate(a.items()):
            ...
            if beautiful:
                ...
                ind = indent
            s.append('%s: %s' % (indent_char * ind + uni_prt(key, encoding, beautiful, indent + 1,
                                                             convertors=convertors), uni_prt(value, encoding, beautiful, indent + 1, convertors=convertors)))
            if i < len(a.items()) - 1:
                if beautiful:
                    s.append(',\n')
                else:
    ...
    def eval(self, value):
        try:
            ...
            if not isinstance(_locals, SortedDict):
                _locals = {}
            v = eval_value(
                value, self.globals, _locals, self.encoding, self.include_env)
            return v
        except Exception:
            print_exc()
...
class Section(SortedDict):
    ...
    def __setitem__(self, key, value, replace=False):
        if self._lazy:
            if key not in self or replace:
                v = Lazy(key, self._root._globals, self._name,
                         self._encoding, self._root._import_env)
            else:
                v = self[key]
            v.add(value)
        else:
        ...
        for f in self.keys():
            ...
            if self._root and self._root._raw:
                ...
                if self._field_flag.get(f, False):
                    ...
                    op = ' = '
                buf = f + op + \
                    uni_prt(self[f], self._encoding, convertors=convertors)
                if len(buf) > 79:
                    buf = f + op + \
                        uni_prt(
                            self[f], self._encoding, True, convertors=convertors)
                print >> out, buf
...
class Ini(SortedDict):
                ...
                if line.startswith(self._commentchar):
                    ...
                    if sec_name.startswith('include:'):
                        ...
                        if os.path.exists(_filename):
                            ...
                            import warnings
                            warnings.warn(
                                Warning("Can't find the file [%s], so just skip it" % _filename), stacklevel=2)
                        continue
                    ...
                    if section is None:
                        raise Exception(
                            "No section found, please define it first in %s file" % self.filename)
                    ...
                    if keyname in self._env:
                        raise KeyError(
                            "Settings key %s is alread defined in env, please change it's name" % keyname)
                    ...
                    if self._lazy:
                        if iden_existed:
                            v = EvalValue(
                                value, filename or self._inifile, lineno, line)
                        else:
                            v = value
                    else:
                        if self._raw:
                            v = RawValue(
                                self._inifile, lineno, value, replace_flag)
                        else:
                            try:
                                v = eval_value(
                                    value, self.env(), self[sec_name], self._encoding, self._import_env)
                            except Exception:
                                print_exc()
                                print dict(self)
                                raise Exception("Converting value (%s) error in %s:%d:%s" % (
                                    value, filename or self._inifile, lineno, line))
    ...
    def add(self, sec_name, comments=None, info=None):
        if sec_name in self:
            ...
            section = Section(
                sec_name, comments, self._encoding, root=self, info=info)
    ...
    def items(self):
        return ((k, self[k]) for k in self.keys() if k not in self._env)
